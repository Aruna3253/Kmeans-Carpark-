# KMeans with Silhouette Score (Automatic Clustering)
import cv2
import numpy as np
import pickle
...
# Silhouette-based optimal cluster selection
def optimal_kmeans_cluster(avg_coords, min_k=2, max_k=10):
    best_score = -1
    best_k = min_k
    for k in range(min_k, min(max_k + 1, len(avg_coords))):
        kmeans = KMeans(n_clusters=k, random_state=42, n_init='auto').fit(avg_coords)
        labels = kmeans.labels_
        score = silhouette_score(avg_coords, labels)
        if score > best_score:
            best_score = score
            best_k = k
    return best_k

# Auto grouping using KMeans + Silhouette
def group_spots_auto(polygons, n_clusters=None):
    avg_coords = np.array([[np.mean([pt[0] for pt in poly])] for poly in polygons])

    if n_clusters is None:
        n_clusters = optimal_kmeans_cluster(avg_coords, min_k=2, max_k=8)

    kmeans = KMeans(n_clusters=n_clusters, random_state=42, n_init='auto').fit(avg_coords)
    labels = kmeans.labels_
...
columns = group_spots_auto(polygon_spots)  # auto-detect best number of columns
...
while True:
    ...
    cv2.imshow("Parking Spot Detection - Auto Columns", overlay)
